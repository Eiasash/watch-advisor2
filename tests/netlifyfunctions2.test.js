...
describe("watch-rec handler", () => {
  ...
  beforeEach(async () => {
    ...
    const { cacheGet } = await import("../netlify/functions/_blobCache.js");
    cacheGet.mockResolvedValue(null);
    const mod = await import("../netlify/functions/watch-rec.js");
    handler = mod.handler;
  });
  ...
  it("returns 500 when API key missing", async () => {
    ...
    const result = await handler({
      httpMethod: "POST",
      body: JSON.stringify({ outfit: {}, watches: [] }),
    });
    expect(result.statusCode).toBe(500);
  });
  ...
  it("returns watch recommendation on success", async () => {
    ...
    const result = await handler({
      httpMethod: "POST",
      body: JSON.stringify({ outfit: { top: { color: "navy" } }, watches: [{ id: "reverso", brand: "JLC", model: "Reverso" }] }),
    });
    expect(result.statusCode).toBe(200);
