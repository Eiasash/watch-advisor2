...
| Tests | 3490 tests, 195 files (Vitest) |
| Version | **1.13.2** |
| Device | OPPO Find X9 Pro |
| Deploys | Auto on push to `main` |
| Last audited | 2026-05-05 R3 (real bug fix). `computeCacheKey` in `netlify/functions/daily-pick.js` only tracked `length + max(created_at)`, so editing a garment's color/formality/category/name/brand in place did NOT change the cache key. The user would edit a garment via GarmentEditor, ask Claude again, and get the pre-edit answer for up to 4 hours (today TTL). Reproducible. Fix: per-garment FNV-1a 32-bit hash over `(id, name, color, formality, type/category, brand)`, XOR-summed across the wardrobe (order-independent — array order doesn't matter for the prompt). Result rendered as 8-char hex appended to gsig. New gsig shape: `${length}-${maxCreatedAt}-${contentHex}`. +8 regression tests in tests/dailyPickCache.test.js pinning each editable field, plus order-independence and `type`-vs-`category` fallback. Test totals: 3482 → 3490, all green. Bumped 1.13.1 → 1.13.2 patch. |
