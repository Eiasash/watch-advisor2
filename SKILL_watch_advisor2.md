...
| Tests | 3,785 tests (Vitest) |
| Version | **1.13.71** |
| Device | OPPO Find X9 Pro |
| Deploys | Auto on push to `main` |
| Last audited | 2026-06-10. v1.13.71 in prod. Site health (via Supabase MCP — snapshot endpoint now user-JWT-only, see §8): garments=124, history=106, orphaned=0, model=claude-sonnet-4-6, auto-heal 9/9 healthy (ran 2026-06-10 05:02 UTC), June tokens 375K in / 24K out ($1.49). **v1.13.69–71:** v1.13.69 (PR#257) daily-pick weather fetch capped at 3s to fit Netlify's 10s budget; v1.13.70 (PR#258) wardrobe-chat merges consecutive same-role history turns + guarantees a leading `user` turn (Claude API 400 guard); v1.13.71 (PR#263) SelfiePanel response-parsing fix. Non-bump: #259 sweater-threshold comments aligned to the 13°C boundary; #260 scoring-weight formula doc corrected + self-improve scan logged; #261 styleDNA pure functions unit-tested (audit-surfaced gap); #262 in-range dep updates — **supabase-js pinned EXACT 2.98.0** (2.108 has a behavior change; do not let any bump past it). ⸻ Previous audit: 2026-06-01. v1.13.64 in prod. Site health: garments=124, history=95, orphaned=0, active straps=43, model=claude-sonnet-4-6. **v1.13.49–55 highlights:** v1.13.49 last-worn strap toggle; v1.13.50 `deleteGarment` soft-delete fix (was hard-DELETEing rows — pre-v1.13.50 phantoms are irrecoverable; never hard-delete, always `exclude_from_wardrobe=true`); v1.13.51 Trade #3 (TAG Monaco → **GP Vintage 1945 Big Date** 25805-11-822-BAEA) + `dial:"ivory"` added to `DIAL_COLOR_MAP`; v1.13.52 Tudor BB41 blue FKM rubber strap added (new default); v1.13.53–54 Trade #4 (Rolex GMT 116710LN → **GO Seventies Chronograph Panorama Date**, +₪3K cash to Eias) + GO default strap corrected to navy alligator (came on gator, not bracelet — `go-seventies-navy-gator`); v1.13.55 (PR #231) GO ref/serial/value corrected from caseback (ref `1-37-02-08-02-62`, serial Nr.0157, marketILS 40000) + empty-history jitter floor 1e-4 (boost was 0 on hash-multiple-of-1000 dates → flaky first-wear test); v1.13.56 Tudor BB41 **black FKM rubber** strap added (`blackbay-black-fkm`, AliExpress, steel deployant clasp) — 2nd rubber strap, distinct from the .52 DayDayWatchband blue FKM; BB41 now 7 active + 2 pending. v1.13.57 bundle strap pick now consumes wear history — gentle strap-**rotation** (recency+frequency) + strap-**health** (finite-life only; bracelets ∞, never penalised) folded into `recommendStrap(watch,outfit,context,weather,history)`; `healthPct` exposed on the rec; shoe scoring/rotation untouched (shoes×0 preserved). v1.13.58 surfaces the rec's `healthPct` as a health pill in WatchDashboard (both strap-rec sites; >60/>30/<30 color scale matching StrapHealth; hidden for bracelets/unworn ≥100%; 'rotate soon' <30%). v1.13.59 layer logic unified to a 3-tier rule (coat<10 / sweater-or-light-layer 10-12 / **no layer >=13**) across getLayerRecommendation, LAYER_TEMP_BRACKETS (<10→1.0/<13→0.8/>=13→0.1), weatherLayerSuggestion, and the builder's sweater gate (14→13) + casual jacket gate (>=13; formal/clinic/shift keep <22 for the blazer). Overrode the 2026-05-07 light-jacket-at-14-21 incident calibration per user rule; 22 stale threshold tests updated. v1.13.60 (PR#240) fixed wardrobe-chat.js system prompt: added a 'WHAT YOU CANNOT DO' honesty boundary (bot may only persist garments/straps/history via tools; must never claim to have noted/saved/'will apply' a rule/setting it has no tool for — it was hallucinating rule persistence), synced its stale layer line to the engine rule, and removed the dead strap-shoe line. v1.13.61 (PR#242): chat roster now generated at request time from WATCH_COLLECTION (wardrobe-chat.js imports the seed; genuine=active non-replica, replica=active replica) so it can't drift on trades — traded pieces auto-drop, new ones auto-appear. v1.13.62 (PR#243): unified strap-health on ONE model — exported computeStrapHealth() from domain/strapLifecycle.js; both buildStrapLifecycle (pill/recommender) and strapStore.getStrapStats (StrapHealth tab) now route through it (canonical lifespans leather350/alligator280/canvas500/nato500/rubber600/bracelet∞). Store gained the alligator distinction. Wear-count still dual-sourced (history vs strap.wearCount) but tracks the same event. Trades #3/#4 are 1-for-1 genuine swaps — active genuine holds at 13; retired 3→5 (Monaco, GMT); 2 pending (Atelier Wen Perception, Fears Brunswick Champagne). v1.13.63 (PR#246): push-subscribe DELETE migrated from x-api-secret (server-only, never reachable from browser → silent 401 on every unsubscribe) to Bearer JWT (`requireUser()`); `unsubscribePush()` now uses `authedFetch()`. v1.13.64 (PR#247): daily-pick.js layer prompt synced to engine: sweater gate 10–12°C (was 10–13), jacket at 13–21°C gated to formal/business context only (smart-casual/sport → no layer), matching `_fillSweaterLayer`/`_fillJacket` `isFormalCtx` behavior. v1.13.65–66 runtime bug fixes (empty-content 400, fn timeouts parallelised, stale-SW React error #300, WeekPlanner hallucination guard). v1.13.67 (this session): **Atelier Wen Perception N°25/50 received in Israel** (the pending P1) → `pending` dropped, now active; active genuine 13→14, pending 2→1 (Fears Brunswick only); added `perception` to both daily-pick watchId enums; first wear `wear-2026-06-03-perception` (bracelet, smart-casual). Replica count is 11 (op_pistachio), correcting stale "10 replica" flavor text in seed comment + prompts. |
...
| **Version bump** | Always bump `package.json` version. Patch/minor/major. Current: **1.13.71**. |
...
| **push-subscribe auth** | v1.13.16 added `requireUser()` to the POST path. v1.13.63 migrated DELETE to the same Bearer JWT gate — the `x-api-secret`/`OPEN_API_KEY` scheme was server-side-only so the browser could never supply it, making every unsubscribe a silent 401. Both POST and DELETE now go through `_auth.js requireUser()`. Client-side: `unsubscribePush()` uses `authedFetch()` (was raw `fetch()`). |
...
| **Layer tiers must agree across display + engine + prompt** | v1.13.18/v1.13.64 — `getLayerRecommendation` (display badge), `_fillSweaterLayer`/`_fillJacket` (`outfitBuilder.js` engine), and the system prompt's layer-logic block (`netlify/functions/daily-pick.js`) all use temperature thresholds for the same decision. **Drift between any two = visible bug** (the 2026-05-07 incident; the daily-pick/engine gap fixed in v1.13.64). Canonical 3-tier model (v1.13.59+): `<10°C coat / 10–12°C sweater+jacket / ≥13°C no warmth layer`. Above 12°C the engine adds a jacket only when `isFormalCtx=true` (formal/business watches), gate `if (temp >= 22) return`. The daily-pick AI prompt reflects this: 13–21°C → formal context: jacket; smart-casual/casual/sport: no layer. When changing any tier boundary, update all three surfaces together and add boundary tests at 10/13/22°C. |
...
### Health check
```
# Browser session only — automation CANNOT call this endpoint (see below).
curl -H "Authorization: Bearer $USER_SESSION_JWT" \
  https://watch-advisor2.netlify.app/.netlify/functions/skill-snapshot
```
**Auth required — user-session JWT ONLY.** `requireUser()` validates via `supabase.auth.getUser(token)` + `ALLOWED_USER_EMAIL` allowlist. The anon/publishable key is **rejected** (`invalid token` — verified live 2026-06-10); only a JWT from Eias's logged-in session passes. Gated since PR #139 (2026-05-03). Consequence: no automation (Claude sandbox, CI, `/update-skill`) can reach this endpoint. For machine-readable ground truth, query Supabase directly (MCP `execute_sql`, project `oaojkanozbfpofbewtfq`):
```sql
SELECT
...
```
Earlier skill versions said "no auth", then "anon JWT or higher" — **both stale**.
