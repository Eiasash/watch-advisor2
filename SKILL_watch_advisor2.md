...
| Tests | 3523 tests, 196 files (Vitest) |
| Version | **1.13.3** |
| Device | OPPO Find X9 Pro |
| Deploys | Auto on push to `main` |
| Last audited | 2026-05-05 R4 (real bug round in the AI ↔ planner recommendation loop). Five distinct fixes, all in `src/components/WeekPlanner.jsx` + new `src/utils/aiPickResolver.js`. (1) Reverso dialSide override silently lost when a strap was set: `enrichedWatch = { ...day.watch, strap }` was spreading the unmodified `day.watch` instead of the dial-overridden `enrichedWatch`. Now spreads `enrichedWatch`. (2) `handleAskClaude` useCallback was missing `rotation`, `watchOverrides`, `strapOverrides`, `straps`, `activeStrap` from its deps — stale closure meant the auto-refit useEffect (which DID have those deps) called the OLD handleAskClaude that read PRE-change state. Result: change strap → AI got asked with the old strap → prose mentions a strap the user didn't pick. Added the missing deps. (3) Inline AI garment-name match was strict equality + lowercase fallback; broke silently on AI responses with whitespace, surrounding quotes, smart quotes, or trailing punctuation (slot fell back to engine pick → outfit didn't match Claude's reasoning). Extracted to `resolveGarmentSlots()` in new `aiPickResolver.js` with `normalizeAiName` helper. (4) Different-watch-mode response had no validation that returned watchId was in the user's collection AND active — hallucinated id silently no-op'd the chip; retired/pending watch could sneak in. Added `validateDifferentWatchPick()`. (5) AI-request race conditions: rapid double-taps fired parallel requests with interleaved state writes; user changing watch mid-flight could be silently reverted by the older response landing after the newer one. Added per-date AbortController in `inflightAbortersRef` — every new request aborts the prior one for that date, response staleness double-checked after `await fetch`, AbortError suppressed from user-facing error UI. Net: +33 unit tests in `tests/aiPickResolver.test.js` covering name normalization, slot resolution, unmatched warnings, and watch validation. Past-corrections synthesis also updated to use `normalizeAiName` on both sides so the more-permissive resolver doesn't generate false-positive "user disagreed" signals from whitespace/punctuation drift. 196 files / 3523 tests, all green. Bumped 1.13.2 → 1.13.3 patch. |
