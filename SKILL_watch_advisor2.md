...
| Tests | 3482 tests, 195 files (Vitest) |
| Version | **1.13.1** |
| Device | OPPO Find X9 Pro |
| Deploys | Auto on push to `main` |
| Last audited | 2026-05-05 R2 (quality/security pass; bumped 1.13.0 → 1.13.1 patch). Net delta: +3 test files / +60 tests, all green. New coverage: tests/auth.test.js (27 tests pinning the JWT + email-allowlist gate from PRs #138-#140 — three-state rollout flag, prod-context auto-enforcement, scary-flag-pair to disable in prod, Bearer header parsing, allowlist case-insensitive trim match, fail-closed misconfig); tests/serviceWorkerRuntime.test.js (20 tests sandboxing public/sw.js and exercising install/activate/fetch/push/message handlers — closes the SW integration gap that was R3 candidate #5); tests/claudeStylistEdge.test.js (13 tests for 4xx/5xx response codes, non-JSON content-type, charset suffix, sync-throw fetch, malformed JSON body, pinnedSlots transformation, sweater + jacket engineOutfit slot transformation). Dep pass: `uuid` overridden to `^14` clears the moderate `GHSA-w5hq-g745-h8pq` audit warning (only used transitively by @netlify/blobs → @netlify/dev-utils — no direct uuid usage in our code); `vitest` + `@vitest/coverage-v8` bumped to 4.1.5 patch. `npm audit` clean. New: scripts/rls-audit.sh — automated 6-check RLS pass (R3 candidate #6) — needs SUPABASE_DB_URL + ALLOWED_USER_EMAIL env vars, exits non-zero on policy weakening, no longer blocked on interactive MCP OAuth. R3 hoist-vi.mock candidate already shipped in commit 39352e2 (verified — zero non-top-level vi.mock calls remain in tests/). |
