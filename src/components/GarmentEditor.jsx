...
export default function GarmentEditor({ garment, onClose }) {
  ...
  function handleSave() {
    updateGarment(garment.id, { name, type: category, color, formality, brand, notes, needsReview: false });
    const updated = garments.map(g => g.id === garment.id
      ? { ...g, name, type: category, color, formality, brand, notes, needsReview: false } : g);
